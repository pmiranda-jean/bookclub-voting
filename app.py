...
from utils.scraper import scrape_goodreads, get_default_book_data
...
if page == "Submit Books":
    ...
    st.info("💡 **Tip:** Enter the book title and author exactly as they appear on Goodreads for best results.")
        ...
        if submitted:
            if submitter_name and book_title and author:
                if book_exists(st.session_state.books, book_title, author):
                    ...
                    with st.spinner("🔍 Fetching book information from Goodreads..."):
                        book_entry = add_book(st.session_state.books, book_title, author, submitter_name)
                        
                        book_data = scrape_goodreads(book_title, author)
                        if book_data:
                            ...
                            st.info("ℹ️ Goodreads data unavailable - you can manually add details later.")
